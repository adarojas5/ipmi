...
boolean mouseActivado() { // verifica que el mouse este dentro de la region especifica (derecha ) 
  if (mouseX >= width/2 && mouseX < width && mouseY >=2 && mouseY < height) {
    return true;
  } else {
    return false;
  }
